...
/**
 * Converte as respostas brutas (JSON/Texto) do Rust para DTOs do Kotlin.
 * Inclui mapeamento de erros amigáveis para falhas do motor nativo.
 */
class RustBridgeMapper {
    ...
    private fun mapFriendlyError(rawError: String): String {
        return when {
            ...
            rawError.contains("MalformedPacket") -> 
                "Falha na análise: o motor Rust encontrou pacotes malformados no arquivo."
    ...
    fun mapSession(raw: String): RustBridgeResult<SessionDto> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            }
            ...
            RustBridgeResult(success = true, data = data)
        } catch (e: Exception) {
            RustBridgeResult(success = false, error = "Erro ao processar dados da sessão: ${e.message}")
        }
    }

    fun mapPacketResult(raw: String): RustBridgeResult<PacketSearchResultDto> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            ...
            val data = if (raw.contains("{")) {
                ...
                val items = lines.filter { it.contains("packet_number") }
                    .map { line ->
            ...
            RustBridgeResult(success = false, error = "Erro ao processar busca de pacotes: ${e.message}")
        }
    }

    fun mapFlowResult(raw: String): RustBridgeResult<FlowSearchResultDto> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            ...
            RustBridgeResult(success = false, error = "Erro ao processar busca de fluxos: ${e.message}")
        }
    }

    fun mapCaptureOverview(raw: String): RustBridgeResult<CaptureOverviewDto> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            ...
            val lines = raw.lines()
            if (lines.isEmpty()) return RustBridgeResult(success = false, error = "Resposta vazia do motor")
            ...
            RustBridgeResult(success = false, error = "Erro ao processar overview: ${e.message}")
        }
    }

    fun mapPackets(raw: String): RustBridgeResult<List<PacketDto>> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            ...
            RustBridgeResult(success = false, error = "Erro ao processar lista de pacotes: ${e.message}")
        }
    }

    fun mapFlows(raw: String): RustBridgeResult<List<FlowDto>> {
        return try {
            if (raw.startsWith("ERROR:")) {
                val msg = raw.substringAfter("ERROR:").trim()
                return RustBridgeResult(success = false, error = mapFriendlyError(msg))
            ...
            RustBridgeResult(success = false, error = "Erro ao processar lista de fluxos: ${e.message}")
        }
    }
