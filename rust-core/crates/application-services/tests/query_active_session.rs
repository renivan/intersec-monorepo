...
#[test]
fn queries_active_packets_and_flows() {
    let file_path = unique_temp_file("app_services_query");
    fs::write(&file_path, build_test_pcap()).unwrap();

    let storage = InMemoryStorage::new();
    let mut app = ApplicationServices::new(storage);

    app.open_capture(file_path.to_str().unwrap(), 1000).unwrap();

    let packet_result = app
        .query_active_packets(&PacketQuery {
    ...
    assert_eq!(packet_result.total_items, 1);

    let flow_result = app
        .query_active_flows(&FlowQuery {
            protocol: Some("DNS".into()),
            host: Some("8.8.8.8".into()),
            port: Some(53),
            text: None,
    ...
    assert_eq!(flow_result.total_items, 1);
