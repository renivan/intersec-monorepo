...
#[test]
fn opens_queries_persists_and_lists_with_android_adapter() {
    let file_path = unique_temp_file("android_adapter_smoke");
    ...
    let packets = adapter
        .query_packets(AndroidPacketQueryInput {
    ...
    let flows = adapter
        .query_flows(AndroidFlowQueryInput {
            protocol: Some("DNS".into()),
            host: Some("8.8.8.8".into()),
            port: Some(53),
            text: None,
    ...
    adapter
        .persist_active("android,adapter", Some("smoke".into()))
        .unwrap();
